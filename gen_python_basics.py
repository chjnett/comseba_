...
/* ===== TAB BAR ===== */
  ...
  .study-view.show { display: block; animation: fadeIn 0.4s ease forwards; }
  .study-card {
    background:var(--bg-panel); border:2px solid var(--border-soft);
    border-radius:12px; padding:20px; margin-bottom:20px;
    box-shadow:0 4px 15px var(--shadow-color);
  }
  .study-card h3 {
    margin-top:0; color:var(--accent-pink); font-size:22px;
    border-bottom:2px dashed var(--border-soft); padding-bottom:10px;
  }
  .study-card p {
    font-size:16px; color:var(--text-color); line-height:1.6; margin:10px 0;
  }
  .study-card ul {
    font-size:16px; color:var(--text-color); line-height:1.6; margin:10px 0; padding-left: 20px;
  }
  .study-card .example-box {
    background:#1E293B; border-radius:8px; padding:15px;
    font-family:'Consolas','Monaco',monospace; color:#A78BFA;
    margin-top:10px; font-size:15px; border-left:4px solid var(--accent-mint); white-space: pre-wrap;
  }
...
<div class="app">
  ...
  <nav class="tab-bar">
    <button class="tab-btn" id="tabStudy" data-tab="study">📖 기초 요약 노트</button>
  ...
  <div id="studyView" class="study-view">
    <div class="map-intro">
      ...
      <p>파이썬의 핵심 개념들을 한눈에 쏙쏙! 코딩하다가 기억이 안 날 때 언제든 여기로 와서 복습해봐요! ✨</p>
    </div>
  ...
  <div class="test-view" id="testView">
    ...
    <div id="mockExamRoom" style="display:none;">
      <div class="problem-card">
        ...
        <div class="speech">🐍 5문제를 모두 풀고 <b>'답안 제출하기'</b> 버튼을 누르면 채점됩니다!</div>
        <div class="speech" id="mockExamConcept" style="margin:20px 0; font-weight:bold; color:var(--text-color); background:var(--bg-panel-2); padding:15px; border-radius:10px;"></div>
...
function showView(name){
  ...
  document.getElementById('studyView').classList.toggle('show', name==='study');
...
const MOCK_EXAM_DESCRIPTIONS = [
  "🎯 [1회차 개념 요약] 화면에 글자를 보여주는 print() 함수와 코드에 메모를 남기는 주석(#), 그리고 글자와 숫자의 차이점을 떠올려 보세요!",
  "🎯 [2회차 개념 요약] 값(데이터)을 담는 상자인 변수(=), 몫(//)과 나머지(%), 그리고 참/거짓을 비교하는 연산자(==, !=, >, <)를 복습해 볼까요?",
  "🎯 [3회차 개념 요약] 여러 개의 값을 하나의 기차처럼 연결하는 리스트([]), 0번부터 시작하는 인덱스, 그리고 append(), del 등을 사용한 리스트 조작을 떠올려 보세요!",
  "🎯 [4회차 개념 요약] '만약 ~라면' 조건을 거는 if문과 들여쓰기, 여러 조건을 연결하는 논리 연산자(and, or, not)를 활용해 논리적인 코드를 짜볼까요?",
  "🎯 [5회차 개념 요약] for문과 while문을 이용한 반복 작업, 그리고 나만의 코드를 만드는 마법의 주문인 함수(def)와 결과값(return)을 확인해 보세요!"
];

const MOCK_EXAMS = [
  [
    {"q": "화면에 '안녕'을 출력하려고 합니다. 알맞은 코드는?", "choices": ["print('안녕')", "show('안녕')", "write('안녕')", "say('안녕')", "display('안녕')"], "answer": 0},
    {"q": "파이썬에서 주석(메모)을 달 때 사용하는 기호는?", "choices": ["#", "//", "<!--", "--", "/*"], "answer": 0},
    {"q": "다음 중 성격이 다른 하나는?", "choices": ["100 (숫자)", "'100' (문자열)", "'안녕'", "'사과'", "'True'"], "answer": 0},
    {"q": "print('파이썬' + '최고') 의 결과는?", "choices": ["파이썬최고", "파이썬 최고", "오류", "아무것도 안나옴", "파이썬+최고"], "answer": 0},
    {"q": "문자열을 나타낼 때 쓸 수 있는 기호는?", "choices": ["따옴표('', \"\")", "괄호(())", "대괄호([])", "중괄호({})", "꺾쇠(<>)"], "answer": 0}
  ],
  [
    {"q": "변수 `age`에 숫자 12를 넣으려면?", "choices": ["age = 12", "age == 12", "12 = age", "age <- 12", "age: 12"], "answer": 0},
    {"q": "파이썬에서 10을 3으로 나눈 '몫'만 구하는 기호는?", "choices": ["//", "/", "%", "mod", "div"], "answer": 0},
    {"q": "파이썬에서 10을 3으로 나눈 '나머지'를 구하는 기호는?", "choices": ["%", "/", "//", "mod", "rem"], "answer": 0},
    {"q": "파이썬에서 '같지 않다'를 뜻하는 비교 기호는?", "choices": ["!=", "==", "<>", "not=", "=/="], "answer": 0},
    {"q": "10 >= 5 의 결과는 참(True)인가요 거짓(False)인가요?", "choices": ["True", "False", "10", "5", "에러"], "answer": 0}
  ],
  [
    {"q": "리스트를 만들 때 사용하는 괄호는?", "choices": ["[ ]", "( )", "{ }", "< >", "없음"], "answer": 0},
    {"q": "리스트 a = ['사과', '바나나', '포도'] 가 있습니다. '사과'를 꺼내는 코드는?", "choices": ["a[0]", "a[1]", "a[사과]", "a.first()", "a[3]"], "answer": 0},
    {"q": "리스트의 맨 끝에 새로운 항목을 추가하는 명령어는?", "choices": ["append()", "add()", "insert()", "push()", "plus()"], "answer": 0},
    {"q": "리스트의 길이를 알아내는 함수는?", "choices": ["len()", "size()", "count()", "length()", "max()"], "answer": 0},
    {"q": "리스트에서 특정 위치의 항목을 삭제하는 키워드는?", "choices": ["del", "remove()", "pop()", "셋 다 가능함", "delete"], "answer": 3}
  ],
  [
    {"q": "if문을 작성할 때 조건 끝에 반드시 찍어야 하는 기호는?", "choices": [": (콜론)", "; (세미콜론)", ". (마침표)", "아무것도 안 찍음", ", (쉼표)"], "answer": 0},
    {"q": "조건이 3개 이상일 때 사용하는 키워드는? (if -> ? -> else)", "choices": ["elif", "else if", "if else", "elseif", "then"], "answer": 0},
    {"q": "두 조건이 '모두' 참이어야 통과시키는 키워드는?", "choices": ["and", "or", "not", "both", "all"], "answer": 0},
    {"q": "두 조건 중 '하나만' 참이어도 통과시키는 키워드는?", "choices": ["or", "and", "not", "any", "some"], "answer": 0},
    {"q": "score = 80 일 때, if score >= 90: 은 어떻게 될까요?", "choices": ["실행되지 않는다", "실행된다", "에러가 난다", "90으로 바뀐다", "프로그램 종료"], "answer": 0}
  ],
  [
    {"q": "정해진 횟수만큼 반복할 때 주로 사용하는 키워드는?", "choices": ["for", "while", "if", "def", "loop"], "answer": 0},
    {"q": "while문을 즉시 멈추고 빠져나오는 명령어는?", "choices": ["break", "stop", "exit", "quit", "return"], "answer": 0},
    {"q": "파이썬에서 나만의 함수를 만들 때 시작하는 단어는?", "choices": ["def", "function", "make", "fun", "func"], "answer": 0},
    {"q": "함수가 계산을 다 하고 결과를 밖으로 던져줄 때 쓰는 키워드는?", "choices": ["return", "give", "send", "throw", "yield"], "answer": 0},
    {"q": "range(1, 4) 가 만들어내는 숫자는?", "choices": ["1, 2, 3", "1, 2, 3, 4", "0, 1, 2, 3", "0, 1, 2, 3, 4", "2, 3, 4"], "answer": 0}
  ]
];
...
function openMockExam(idx){
  ...
  document.getElementById('mockExamConcept').textContent = MOCK_EXAM_DESCRIPTIONS[idx];
...
document.getElementById('submitTestBtn').addEventListener('click', ()=>{
  ...
  document.getElementById('testScoreDisplay').textContent = `🎉 내 점수: ${score}점 (${correctCount}/5)`;
...
function renderStudyNotes() {
  ...
  STUDY_NOTES.forEach(note => {
    ...
    if(note.code){
      html += `<div class="example-box">${note.code}</div>`;
    }
    html += `</div>`;
...
const STUDY_NOTES = [
  {
    title: "📦 1. 변수와 기본 자료형 (Variables & Types)",
    content: "<b>변수</b>란 데이터를 담아두는 '이름표가 붙은 상자'입니다.<br>파이썬에서는 상자 안에 숫자나 문자를 넣고 나중에 꺼내 쓸 수 있어요.",
    list: [
      "숫자 (Integer, Float): 계산이 가능한 진짜 숫자 (예: 100, 3.14)",
      "문자열 (String): 따옴표로 감싼 글자 (예: '안녕', \"사과\")",
      "불리언 (Boolean): 참과 거짓 (True, False)"
    ],
    code: "name = '지민'\\nage = 12\\nis_student = True\\nprint(name, age)"
  },
  {
    title: "➕ 2. 파이썬의 연산자 (Operators)",
    content: "파이썬으로 더하기, 빼기 등 다양한 계산을 할 수 있습니다.",
    list: [
      "기본 사칙연산: 더하기(+), 빼기(-), 곱하기(*), 나누기(/)",
      "특수 연산: 몫(//), 나머지(%), 제곱(**)",
      "비교 연산: 같다(==), 다르다(!=), 크다(>), 작다(<) -> 결과는 True 또는 False!"
    ],
    code: "print(10 + 5)    # 15\\nprint(10 // 3)   # 3 (몫)\\nprint(10 % 3)    # 1 (나머지)\\nprint(10 == 10)  # True"
  },
  {
    title: "🚂 3. 리스트 (Lists)",
    content: "여러 개의 값을 한꺼번에 보관할 수 있는 <b>기차</b> 같은 구조입니다.<br>각 칸에는 0번부터 번호(인덱스)가 매겨져 있어요.",
    list: [
      "리스트 만들기: 대괄호 [ ] 를 사용해요.",
      "인덱싱: a[0] (첫 번째), a[-1] (마지막)",
      "항목 추가: append() (맨 끝에 추가)",
      "항목 삭제: del a[0] (0번 삭제), remove('사과') (값으로 삭제)"
    ],
    code: "fruits = ['사과', '바나나']\\nfruits.append('포도')\\nprint(fruits[0])  # 사과\\ndel fruits[1]     # 바나나 삭제"
  },
  {
    title: "🚦 4. 조건문 (If Statement)",
    content: "<b>'만약 ~라면'</b>이라는 뜻으로, 조건에 따라 다른 코드를 실행합니다.",
    list: [
      "if 조건:: 조건이 참(True)일 때 실행",
      "elif 조건:: 위의 조건이 거짓이고 새 조건이 참일 때 실행",
      "else:: 모든 조건이 거짓일 때 실행",
      "중요! 조건문 아래의 코드는 반드시 들여쓰기(스페이스 4칸)를 해야 합니다.",
      "논리 연산: 둘 다 참이면(and), 하나라도 참이면(or), 반대로(not)"
    ],
    code: "score = 85\\nif score >= 90:\\n    print('A학점')\\nelif score >= 80:\\n    print('B학점')\\nelse:\\n    print('C학점')"
  },
  {
    title: "🔁 5. 반복문 (Loops)",
    content: "같은 작업을 여러 번 반복할 때 사용합니다.",
    list: [
      "for문: 정해진 횟수나 범위 안에서 반복합니다. (range, 리스트 활용)",
      "while문: 조건이 참(True)인 동안 계속해서 반복합니다.",
      "break: 반복문을 중간에 멈추고 완전히 빠져나옵니다.",
      "continue: 이번 차례만 건너뛰고 다음 반복으로 넘어갑니다."
    ],
    code: "for i in range(3):\\n    print('안녕!')  # 3번 출력\\n\\nwhile True:\\n    print('무한반복')\\n    break  # 바로 탈출!"
  },
  {
    title: "🪄 6. 함수 (Functions)",
    content: "자주 사용하는 코드를 묶어놓은 <b>'나만의 마법 주문'</b>입니다.",
    list: [
      "만들기: def 함수이름(매개변수):",
      "매개변수: 함수에 전달하는 입력값입니다.",
      "결과값 반환: return 을 사용해 계산 결과를 함수 밖으로 던져줍니다."
    ],
    code: "def add(a, b):\\n    return a + b\\n\\nresult = add(3, 4)\\nprint(result)  # 7"
  }
];
